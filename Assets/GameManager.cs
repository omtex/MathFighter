...
public class GameManager : MonoBehaviour
{
    ...
    [SerializeField] TMP_Text InfoText; // genel anlarda ekrana ��kan yaz�
    ...
    private void Start()
    {
        StartPanelGame(); // Main Menu(�leride farkl� sahneye aktar�labilir)
        GetHighScore();
        Audio_Source = GetComponent<AudioSource>();   
    }
    public void StartGame() // Oyun ba�latma
    {
        StartCoroutine(StartingTimer());
    }
    private IEnumerator StartingTimer() // Oyun ba�lamadan �nce s�re say�l�r
    {
    ...
    public void EndGame(bool IsRestartScene) // Oyun durdurulur. Restart ise sahne yenilenir. De�il ise oyun resetlenir
    {
        if (IsRestartScene)
        {
            HighScoreControl();
    ...
    public void SetDifficulty(DifficultyGame gameDiff) // UI da zorluk se�erken ki backend
    {
    ...
    public void ResetGame(bool isToMenu) // Belli durumlarda oyundaki sistemleri s�f�rlar durdurur.
    {
        IsGamePaused  = false;
        currentBaloonSpeedIncrease = BaloonSpeedIncrease;
        currentBaloonTime = BalloonTime;
    ...
    #endregion
    private IEnumerator BaloonSpawn() // Balonlar spawn olmaya ba�lar. 
    {
        ...
        while (IsGameRun)
        {
            ...
            Button newBaloon = Instantiate(PrefabBalonn, SpawnPoints[spawnIndex].transform.position,Quaternion.identity); // balonu olu�tur
            ...
            int rndResult = UnityEngine.Random.Range(0, 2); // do�ru cevap yada yanl�� cevab� entegre et
            int result = 0;
            btn.TrueValue = value_number; 
            if (rndResult == 0) // do�ru de�eri ata
            {
            ...
            else if(rndResult == 1) // yanl�� de�er ata
            {
    ...
    public void ClearBaloons() // T�m balonlar silinir
    {
   ...
   public void SetValue() // yeni say� belirlenir
    {
    ...
    public void SelectControl(int value, Button btn) // Topa(Balona) t�kland���nda kontrol ger�ekle�ir
    {
    ...
    private IEnumerator ComboEffect() // Combo ger�ekle�ti�inde efekt yap�l�r.
    {
    ...
    private IEnumerator InfoTextCreate(string text,Color clr,float delay) // Ekrana yaz� g�sterir
    {
    ...
    public void RestartTheGame() // Game Over Ekran�nda
    {
